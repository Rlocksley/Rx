...
struct Actors {
 Actors(flecs::world& world) {
        ...
        for(int i = 0; i < corners.size(); i++) {
            ...
            (glm::vec3(corners[i].x * 60.f, 60.f, corners[i].y * 60.f),   // position above center
        ...
        Rx::Shape::ColorCube cube(glm::vec3(1.f, 1.f, 1.f), glm::vec4(2.f, 2.f, 2.f,1.f));
        ...
        for(int i = -5 ; i < 5; i++){
            for(int j = -5; j < 5; j++){
                ...
                e.add<Rx::Component::VkShadowColorModelDescriptorSet>();
