...
struct Actors {
 Actors(flecs::world& world) {
        ...
        floor.set<Rx::Component::BoxCollider>({
            .halfExtent = glm::vec3(100.f, 1.0f, 100.f), // Large enough to cover the floor
        });
        ...
        for(int i = -5 ; i < 5; i++){
            for(int j = -5; j < 5; j++){
                for(int k = 0; k < 10; k++){
                    ...
                    e.set<Rx::Component::Transform>({ glm::vec3(1.f), 0.f, glm::vec3(0.f, 1.f, 0.f), glm::vec3(0.f,2.f,0.f) + glm::vec3(i * 9.f,  k * 9.f, j * 9.f) });
                    e.add<ToMove>();
                }
            }
        }
        ...
        // Add WoW Character with shadow support (similar to example8)
        auto skeletonAsset = Rx::Asset::loadSkeletonModel("C:/Users/robry/Desktop/3DModels/wizard.skelmod.rx", world, "WizardSkeletonModelAsset", 100);
        
        auto character = Rx::Asset::createSkeletonModelInstance(world, skeletonAsset, "Wizard");
